const form=document.querySelector('#scanForm'),input=document.querySelector('#photo'),storedInput=document.querySelector('#storedPhoto'),preview=document.querySelector('#preview'),prompt=document.querySelector('#cameraPrompt'),status=document.querySelector('#status'),result=document.querySelector('#result'),button=document.querySelector('#scanButton');
...
let lastResult,supabaseClient,session,selectedFile;
...
function selectPhoto(file){if(!file)return;selectedFile=file;preview.src=URL.createObjectURL(file);preview.hidden=false;prompt.hidden=true;result.hidden=true;status.textContent='Photo ready to scan.'}input.addEventListener('change',()=>selectPhoto(input.files[0]));storedInput.addEventListener('change',()=>selectPhoto(storedInput.files[0]));
form.addEventListener('submit',async e=>{e.preventDefault();if(!selectedFile){status.textContent='Take a photo or choose one saved first.';return;}if(supabaseClient&&!session){authDialog.showModal();status.textContent='Sign in by email to scan and save your wines.';return}button.disabled=true;button.textContent='Researching this wine…';status.textContent='Reading the label, then finding current reviews, ratings and prices.';result.hidden=true;try{const body=new FormData();body.append('photo',selectedFile);const headers=session?{Authorization:`Bearer ${session.access_token}`}:{ };const response=await fetch('/api/identify',{method:'POST',headers,body});const data=await response.json();if(!response.ok)throw new Error(data.error);lastResult=data;render(data);status.textContent='Wine identified and researched.'}catch(err){status.textContent=err.message}finally{button.disabled=false;button.textContent='Show me this wine'}});
